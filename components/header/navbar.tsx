      ...
      <motion.div
        className="fixed top-2 left-1/2 -translate-x-1/2 h-[6.5rem] w-11/12 rounded-none border 
                    border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shadow-black/[0.03] 
                      backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:w-[40rem] sm:rounded-full dark:bg-white dark:border-white/40 dark:bg-opacity-75"
          ...
          {links.map((link) => (
            <motion.li
              ...
              className={clsx(
                ...
                { 'text-black dark:text-white': link.name === activeSection }
              )}
            ...
            >
              <Link href={link.hash}>
                ...
                {link.name === activeSection && (
                  <motion.span
                    className="bg-gray-200/60 rounded-full absolute inset-1 -z-10 dark:bg-black"
